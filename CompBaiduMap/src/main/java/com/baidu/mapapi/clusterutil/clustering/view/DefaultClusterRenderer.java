...
//        });
    ...
    public interface IMarkerViewListener{
        View getMarkerView(String nums, Cluster cluster);
        int getBucket(int nums, Cluster cluster);
    }
    public void setMarkerViewListener(IMarkerViewListener markerViewListener){
        this.markerViewListener = markerViewListener;
    }
    private IMarkerViewListener markerViewListener;
    /**
    ...
    protected void onBeforeClusterRendered(Cluster<T> cluster, MarkerOptions markerOptions) {
        ...
        int bucket_key = bucket;
        if(markerViewListener != null){
            bucket_key = markerViewListener.getBucket(bucket,cluster);
        }
        BitmapDescriptor descriptor = mIcons.get(bucket_key);
        if (descriptor == null) {
            if(markerViewListener == null){
                mColoredCircleBackground.getPaint().setColor(getColor(bucket));
                descriptor = BitmapDescriptorFactory.fromBitmap(mIconGenerator.makeIcon(getClusterText(bucket)));
            }else{
                descriptor = BitmapDescriptorFactory.fromView(markerViewListener.getMarkerView(getClusterText(bucket),cluster));
            }
            mIcons.put(bucket_key, descriptor);
        }
                ...
                for (T item : cluster.getItems()) {
                    ...
                    if (marker == null) {
                        ...
                        if (animateFrom != null) {
                            ...
                            if(item.getBitmapDescriptor() != null){
