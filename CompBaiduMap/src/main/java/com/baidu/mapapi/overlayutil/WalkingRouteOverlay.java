    ...
    public final List<OverlayOptions> getOverlayOptions() {
        ...
        if (mRouteLine.getAllStep() != null
            ...
            for (WalkingRouteLine.WalkingStep step : mRouteLine.getAllStep()) {
                ...
                if (step.getEntrance() != null && showNodeIcon()) {
                    overlayList.add((new MarkerOptions())
                            ...
                            .icon(BitmapDescriptorFactory
                                    .fromAssetWithDpi("Icon_direction.png")));
                }

                // 最后路段绘制出口点
                if (mRouteLine.getAllStep().indexOf(step) == (mRouteLine
                        .getAllStep().size() - 1) && step.getExit() != null && showNodeIcon()) {
                    overlayList.add((new MarkerOptions())
        ...
        if (mRouteLine.getAllStep() != null
            ...
            for (WalkingRouteLine.WalkingStep step : mRouteLine.getAllStep()) {
                ...
                if (watPoints != null) {
                    ...
                    overlayList.add(new PolylineOptions().points(points).width((getLineWidth() != 0 ? getLineWidth() : 10))
    ...
    public final boolean onMarkerClick(Marker marker) {
        for (Overlay mMarker : mOverlayList) {
            if (mMarker instanceof Marker && mMarker.equals(marker)) {
                ...
                if (getMarkViewInterface() != null) {
                    View popView = getMarkViewInterface().onMarkViewClick(marker);
                    if (popView != null) {
                        if (popView.getTag() != null) {//通过tag设置popView的高度
                            int position = 0;
                            try {
                                position = (int) popView.getTag();
                            } catch (Exception e) {
                            }
                            InfoWindow mInfoWindow = new InfoWindow(popView, marker.getPosition(), position);
